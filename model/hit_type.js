...
module.exports = function (conf) {
  ...
  HITType.prototype.validate = function (v) {
    ...
    if (this.reward.valid && !this.reward.valid()) this.reward.errors.forEach(function (err) {
  ...
  HITType.prototype.create = function (callback) {
    ...
    request('AWSMechanicalTurkRequester', 'RegisterHITType', 'POST', options, function (err, response) {
      ...
      if (remoteErrors) {
        callback(new Error(remoteErrors.join(', ')));
        return;
      }
      delete response.RegisterHITTypeResult.Request;

      if (!self.nodeExists(['RegisterHITTypeResult', 'HITTypeId'], response)) {
        callback([new Error('No "RegisterHITTypeResult > HITTypeId" node on RegisterHITType response')]);
        return;
      }
  ...
  ret.setNotification = function setNotification(hitTypeId, notification, active, callback) {
    ...
    if (!notification.valid()) {
      callback(new Error('invalid Notification: ' + notification.errors.join(', ')));
      return;
    }
